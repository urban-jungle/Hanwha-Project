/*****************************************************************************
    ...
    Purpose:
        Get synchronous data from multiple MCC 118 devices.

    Description:
        This example demonstrates acquiring data synchronously from multiple
        MCC 118 devices.  This is done using the external clock and
        external trigger scan options.  The CLK terminals must be connected
        together on all MCC 118 devices being used and an external trigger
        source must be provided to the TRIG terminal on the master MCC 118
        device.  The OPTS_EXTCLOCK scan option is set on all of the MCC 118
...
int main(void)
{
    ...
    int buffer_size = samples_per_channel * mcc118_num_channels;
    ...
    int chans[DEVICE_COUNT][mcc118_num_channels];
    int device = 0;
    int sample_idx = 0;
    int i = 0;
    ...
    result = mcc118_trigger_mode(address[MASTER], trigger_mode);
    STOP_ON_ERROR(result);
    ...
    printf("\nMCC 118 multiple device example using external clock and "
    ...
    while (is_running)
    {
        ...
        {
            // Read data
            result = mcc118_a_in_scan_read(address[device], 
